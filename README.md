    ...
    <tr>
      <td align="center">
        ...
        <span style="font-size: 13px; text-align: left;">
          ...
          Upstash Redis 기반 AI 요청 제한 구현 경험 (현재 버전 제외)<br/>
          실시간 채팅 기능<br/>
          모든 게시글 관련 페이지<br/>
          로그인/회원가입 페이지<br/>
...
## 🧩 개발 경험 — Upstash Redis 기반 AI 요청 제한

**담당: 이준희**

AI 챗봇의 반복 호출에 따른 API 비용과 서버 부하를 제어하기 위해 Upstash Redis 기반 요청 제한을 구현했습니다.

- 요청에 전달된 사용자 ID를 기준으로 요청 횟수와 차단 만료 시각을 별도 키로 관리했습니다.
- `INCR`로 요청 횟수를 증가시키고, 첫 요청 시 `EXPIRE`로 집계 키의 유효기간을 설정했습니다.
- 요청 한도를 넘으면 TTL이 있는 차단 키를 저장하고, AI 호출 전에 HTTP `429` 응답을 반환했습니다.
- 차단 중 다시 요청하면 남은 대기 시간을 응답에 포함해 재시도 시점을 안내했습니다.

이 과정에서 Redis의 카운터와 TTL을 이용해 요청 상태를 관리하고, AI 기능의 호출 비용을 고려한 제한 로직을 구현하는 경험을 쌓았습니다.

> **현재 상태:** 해당 제한 로직은 이전 버전에 구현했으며, 현재 챗봇 API에서는 제외한 상태입니다. 현재 적용 기능과 구분해 개발 이력으로 기록합니다. [구현 커밋](https://github.com/Lineon24/PTspoon/commit/001576b) · [제외된 변경 내역](https://github.com/Lineon24/PTspoon/commit/1f30af0)
